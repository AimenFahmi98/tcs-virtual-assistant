  ...
  return (
    <div className="flex-1 overflow-auto">
      <div className={`flex flex-col gap-4 p-4 ${className}`}>
        {questions.length === 0 ? (
          <div className="text-center italic text-text_light">
            ✨ Welcome to the App Agent! ✨
            <br />
            Let me perform actions on your behalf!
          </div>
        ) : (
          questions.map((question) => (
            <div key={question.id}>
              <Question
                className={
                  "mb-4 w-[90%] justify-self-end rounded-3xl bg-primary_darker px-4 py-2 text-sm"
                }
              >
                {question.content}
              </Question>
